...
class OutlierMainWindow(QMainWindow):
    ...
    def _create_widgets(self):
        ...
        # self.proj_cutoff_spin = QDoubleSpinBox() # Removed
    ...
    def _create_layouts(self):
        ...
        # param_layout.addRow("Projection Z-Cutoff (Filtering Only):", self.proj_cutoff_spin) # Removed
    ...
    def _initialize_plots(self):
        ...
        self.sg_sr_scatter_included = pg.ScatterPlotItem(pen=pg.mkPen(None), brush=pg.mkBrush(128, 0, 128, 150), size=5, name='Included (Time|Resid Filter)')
        self.sg_sr_scatter_outliers = pg.ScatterPlotItem(pen=pg.mkPen('r'), brush=pg.mkBrush(255, 0, 0, 200), size=7, symbol='o', name='Outlier (Time|Resid Filter)')
    ...
    # Updated slot signature (removed proj_mask)
    @pyqtSlot(pd.Series, tuple, pd.Series, tuple)
    def on_worker_outliers_detected(self, time_mask, time_params, resid_mask, resid_params):
    ...
    def handle_run_pause_resume(self):
        if not self.is_running:
            ...
            time_cutoff = self.time_cutoff_spin.value()
            # proj_cutoff = self.proj_cutoff_spin.value() # Removed
            resid_cutoff = self.resid_cutoff_spin.value()
            ...
            self.thread = QThread()
            # Pass only relevant cutoffs to worker
            self.worker = Worker(self.input_dir, self.output_dir, time_cutoff, resid_cutoff)
            ...
            if self.worker:
                ...
                # proj_cutoff = self.proj_cutoff_spin.value() # Removed
                resid_cutoff = self.resid_cutoff_spin.value()
                # Update worker with relevant params
                self.worker.update_parameters(time_cutoff, resid_cutoff)
                self.worker.resume()
                self.is_paused = False
                self.run_pause_button.setText("Pause")
                self.set_controls_enabled(False)
    ...
    def set_controls_enabled(self, enabled):
        ...
        # self.proj_cutoff_spin.setEnabled(enabled) # Removed
